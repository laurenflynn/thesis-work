...
Here we will remove the MLH1 status from the probands so that PanelPRO can predict the status.
...
Next we want to make a function that will filter to the affected family members in addition to the proband.
...
affectedFamilyMembers <- function(ped){
  proband <- ped %>% filter(isProband==1)
  relatives <- ped %>% filter(isProband==0) %>% filter(isAffAny==1)
...
describeFamilies <- function(fams){
  ...
  print(paste0("Average family size: ", mean(sizeOfFamily)))
  print("Summary of family sizes")
  print(summary(sizeOfFamily))
...
considered_fams = families
for(i in 1:length(families)){
  currFam = families[[i]] %>% filter(isProband==1)
  #remove mlh1 info from the proband
  proband = currFam %>% filter(isProband==1) 
  proband$MLH1 = NA
  View(currFam)
...
for(i in 1:length(families)){
  ...
  id = as.character(probandIDS[i])
  ambryFirstDegreeCarrierRisk[[i]] <- out$posterior.prob[[id]]$estimate[2]
  outputsAmbryFirstDegree[[i]] = out#DF
}
...
| Variable | Description | 
| ----- | ---- | 
| fullCarrierRisk | Carrier Risk for MLH1 given the full family pedigree |
| outputsFull | Outputs of PanelPRO for full family pedigree |
| firstDegreeCarrierRisk | Carrier Risk for MLH1 given the first degree family pedigree |
| outputsFirstDegree | Car
