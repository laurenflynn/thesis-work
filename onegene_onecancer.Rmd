...
** Note: should we be removing MLH1 status from family members? I think probably not
Update: doing this now only in the masking section
...
## Masked Information from Unaffected
```{r}
...
```{r test affected family}
affectedFamilyMembersAll <- function(ped){
  proband <- ped %>% filter(isProband==1)
  relatives <- ped %>% filter(isAffAny==1)
  relatives <- relatives %>% filter(isProband==0)
  affectedFam <- rbind(proband, relatives)
  return(affectedFam)
}

affectedOnly <- lapply(families, affectedFamilyMembersAll)


outputsAffectedOnly <- list ()
affectedOnlyCarrierRisk <- list()
#considers probands and their parents and affected family members
for(i in 1:length(families)){
 # browser()
  out = PanelPRO::PanelPRO(affectedOnly[[i]], genes="MLH1", cancers="Colorectal", debug = TRUE)
  id = as.character(probandIDS[i])
  affectedOnlyCarrierRisk [[i]] <- out$posterior.prob[[id]]$estimate[2]
  outputsAffectedOnly[[i]] = out#DF
}

```

Issue: as we can see, simply running the model based on affected family members and the proband doesn't work because it breaks links in the family tree which causes errors eg: "ID 1 has been removed from pedigree since they are not connected with any proband."
