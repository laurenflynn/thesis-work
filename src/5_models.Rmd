...
modelSummaryA1 <- modelSummary
print(xtable(modelSummaryA1, digits=4), file = "a1_models.tex")
```
...
modelSummary <- rbind.fill(modelSummary, modelSummary4)
modelSummaryU1 <- modelSummary
print(xtable(modelSummaryU1, digits =4), file = "u1_models.tex")
...
# Clinical Threshold Proportions
```{r}
clinicallySignificantFull <- summaryTable %>% filter(fullCarrierRisk >0.05)
clinicallySignificantMasked <- summaryTable %>% filter(carrierRiskUnaffectedInfoMasked >0.05)
print(nrow(clinicallySignificantFull))
print(nrow(clinicallySignificantMasked))
onlySigFromMasked <- anti_join(clinicallySignificantMasked, clinicallySignificantFull, by = "famID")
print(nrow(onlySigFromMasked))
```
